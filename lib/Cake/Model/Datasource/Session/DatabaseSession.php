  ...
  public function write($id, $data) {
      ...
      if (!$id) {
       ...
       // print_r($record);
        ...
        $result = $this->_model->find("all", array('conditions' => array('id' => $id)));
       // pr($result);
        $flag = NULL;
        if (empty($result)) {
            $flag = $this->_model->save($record);
        } else {
       //   $flag = $this->_model->updateAll($record, array('id' => $id));
             $flag = $this->_model->query("update ngdrstab_mst_sessions set id=? ,data=?,expires=? where id=?",array($id,$data,$expires,$id));
        }
         //   pr($flag);
        return $flag;
         } catch (Exception $exc) {
