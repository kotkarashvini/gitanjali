 ...
 */
	protected function _findUser($username, $password = null) {
		...
		$user = $result[$model];
		if ($password !== null) {
                    $temp_pass=$user[$fields['password']];
                    $saltNew =    CakeSession::read('session_salt');
                    $temp_pass1 = Security::hash($temp_pass,'sha256', $saltNew);
               
//                  if (!$this->passwordHasher()->check($password, $temp_pass)) {
                    if (!($temp_pass1===$password)) {
                            return false;
                    }
