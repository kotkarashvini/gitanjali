...
class evalcalculation extends AppModel {
    ...
    //put your code here
    //for subrule
    public function calculation_part($usageitemlist, $evalformula, $rate, $fixedrate, $fixedrate_RR4, $fixedrate_extra, $additionalrate, $additionalrate2, $comparisonrate, $formdata) {
        ...
        $evalformula = str_replace('RR3', $fixedrate_extra, $evalformula);
        $evalformula = str_replace('RR4', $fixedrate_RR4, $evalformula);
        return $evalformula;
    }

    public function rate_factor_effect($rateflag = NULL, $formdata = NULL, $rate = NULL, $additionalrate = NULL, $additionalrate2 = NULL, $fixedrate = NULL, $fixedrate_extra = NULL, $comparisonrate = NULL, $rate_rivision_formula = NULL, $ratefactorarray = NULL) {
        $effectiverate = 0;
        $ratefactor = NULL;
        ...
        if ($rateflag == 'RRR') {
          //  echo 'hi';
            ...
            $rate_rivision_formula = str_replace('RR3', $fixedrate_extra, $rate_rivision_formula);
//pr($formdata);
//pr($ratefactor);exit;
            if ($formdata['rate_revision_flag'] == 'Y'  && $rate_rivision_formula != NULL) {
                $rivision_rate = eval("return ($rate_rivision_formula);");
                if ($ratefactor != NULL) {
                    $effectiverate = $ratefactor * $rivision_rate;
                } else {
                    $effectiverate = $rivision_rate;
                }
        ...
        } else if ($rateflag == 'RR2' && $fixedrate != NULL) {
            ...
            $rate_rivision_formula = str_replace('RR5', $additionalrate2, $rate_rivision_formula);
            $rate_rivision_formula = str_replace('RR3', $fixedrate_extra, $rate_rivision_formula);
            if ($formdata['rate_revision_flag'] == 'Y' && $fixedrate != NULL && $rate_rivision_formula != NULL) {
                $rivision_rate = eval("return ($rate_rivision_formula);");
                if ($ratefactor != NULL) {
                    $effectiverate = $ratefactor * $rivision_rate;
                } else {
        ...
        } else if ($rateflag == 'RR3' && $fixedrate_extra != NULL) {
            ...
            $rate_rivision_formula = str_replace('RR5', $additionalrate2, $rate_rivision_formula);
            $rate_rivision_formula = str_replace('RR3', $fixedrate_extra, $rate_rivision_formula);
            if ($formdata['rate_revision_flag'] == 'Y' && $fixedrate_extra != NULL && $rate_rivision_formula != NULL) {
                $rivision_rate = eval("return ($rate_rivision_formula);");
                if ($ratefactor != NULL) {
                    $effectiverate = $ratefactor * $rivision_rate;
                } else {
                ...
                $effectiverate = eval("return (" . $fixedrate_extra . '*' . $ratefactor . ");");
            } else {
                $effectiverate = $fixedrate_extra;
            }
        } else if ($rateflag == 'ABE' && $comparisonrate != NULL) {
            ...
            $rate_rivision_formula = str_replace('RR5', $additionalrate2, $rate_rivision_formula);
            $rate_rivision_formula = str_replace('RR3', $fixedrate_extra, $rate_rivision_formula);
            if ($formdata['rate_revision_flag'] == 'Y' && $comparisonrate != NULL && $rate_rivision_formula != NULL) {
                $rivision_rate = eval("return ($rate_rivision_formula);");
                if ($ratefactor != NULL) {
                    $effectiverate = $ratefactor * $rivision_rate;
                } else {
    ...
    public function multiplecalculation($formdata = NULL, $json2array = NULL, $subrulecondition = NULL, $rate = NULL, $fixedrate = NULL, $fixedrate_extra = NULL, $comparisonrate = NULL, $additionalrate = NULL, $additionalrate2 = NULL, $ratefactorarray = NULL) {
        ...
        $fixedrate_RR4 = $fixedrate;  // Contruction Rate Without Depriciation ie.RR4
        // replace rate in conditions 
        $ratefields = array('RRR' => $rate, 'RR1' => $additionalrate, 'RR2' => $fixedrate, 'ABE' => $comparisonrate, 'RR5' => $additionalrate2, 'RR3' => $fixedrate_extra, 'RR4' => $fixedrate_RR4);
...
//echo $rate;exit;
                    $rate = $this->rate_factor_effect($rateflag = 'RRR', $formdata, $rate, $additionalrate, $additionalrate2, $fixedrate, $fixedrate_extra, $comparisonrate, $rate_rivision_formula, $ratefactorarray);
                    $fixedrate = $this->rate_factor_effect($rateflag = 'RR2', $formdata, $rate, $additionalrate, $additionalrate2, $fixedrate, $fixedrate_extra, $comparisonrate, $rate_rivision_formula, $ratefactorarray);

                    $evalformula = $this->calculation_part($json2array['usageitemlist'], $evalformula, $rate, $fixedrate, $fixedrate_RR4, $fixedrate_extra, $additionalrate, $additionalrate2, $comparisonrate, $formdata);
...
//        pr($formdata);exit;
        ...
        if ($subrulecondition['max_value_condition_flag'] == "Y") {
            ...
            $rate_rivision_formulamx = $subrulecondition['rate_revision_formula_max'];
            $comparisonrate = $this->rate_factor_effect($rateflag = 'ABE', $formdata, $rate, $additionalrate, $additionalrate2, $fixedrate, $fixedrate_extra, $comparisonrate, $rate_rivision_formulamx, $ratefactorarray);
           
            $maxvalformula = $this->calculation_part($json2array['usageitemlist'], $maxvalformula, $rate, $fixedrate, $fixedrate_RR4, $fixedrate_extra, $additionalrate, $additionalrate2, $comparisonrate, $formdata);
...
//                exit;
    ...
    public function singlecalculation($formdata = NULL, $json2array = NULL, $rate = NULL, $fixedrate = NULL, $jsonarray = NULL, $comparisonrate = NULL, $additionalrate = NULL, $additionalrate2 = NULL, $ratefactorarray = NULL) {
        ...
        if ($json2array['evalrule']['max_value_condition_flag'] == "Y") {
            ...
            $maxvalformula = $this->calculation_part($jsonarray['usageitemlist'], $evalformula, $rate, $fixedrate, $additionalrate, $additionalrate2, $comparisonrate, $formdata);
            $this->calculation_part($jsonarray['usageitemlist'], $maxvalformula, $rate, $fixedrate, $additionalrate, $comparisonrate, $formdata);
            $maxvalformula = str_replace('ABE', $comparisonrate, $maxvalformula);
            $maxvalresult = eval("return ($maxvalformula);");
