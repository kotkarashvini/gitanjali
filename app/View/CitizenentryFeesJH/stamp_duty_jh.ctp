    ...
    $(document).ready(function () {
        ...
        $('input:radio[name="data[frm][exemption_flag]"]').change(function () {
            ...
            if ($(this).val() == 'Y') {
                ...
                $.post(host + 'FeesJH/delete_fee_exemption_jh', {}, function (Data) {
                    getExemption();
                    $('#exemption_amt').val(0);
                    $('#exemption_div').hide();
                });
...
//        ($('#sd_adj_flag').val() == 'Y') ? $('#sd_adj_div').show() : $('#sd_adj_div').hide();
        ...
        if ($("#feeRule_id option:selected").val()) {
            ...
            $.post('<?php echo $this->webroot; ?>CitizenentryFeesJH/get_exmption_rule_jh', {fee_rule_id: fee_rule_id}, function (data)
            {
...
<?php } ?>
        ...
        $('#exemption_feeRule_id').change(function () {
            ...
            if ($(this).val()) {
                $.post(host + 'FeesJH/get_article_fee_rule_item_input_jh', {feerule_id: $(this).val(), exmption: 'Y'}, function (ExmItemData) {
        ...
        $("#btnCalculateExemption").click(function () {
            var exm_arg = {doc_token_no: $('#token_no').val(), exm: 'Y', fee_rule_id: $('#exemption_feeRule_id').val(), FAS: $('#onlineSDAmount').val(), FAG: $('#FAG').val(), FAF: $('#FAF').val(),FBG: $('#FBG').val(), FBF: $('#FBF').val(),FBP: $('#FBP').val(),FCY: $('#FCY').val(),FCT: $('#FCT').val(),FBN: $('#FBN').val(),FDC: $('#FDC').val(),FDD: $('#FDD').val(),FCW: $('#FCW').val(),FDF: $('#FDF').val(), property_id: $('#exm_prop_id').val()};
            $.ajax({
                type: 'post',
                url: host + 'FeesJH/calculate_fees_jh',
                data: exm_arg,
        ...
        $('#btnNext').click(function () {
            ...
            $.post(host + 'CitizenentryFeesJH/get_fees_falc_ids_jh', {doc_token_no: $('#token_no').val()}, function (data) {
                var prop_count = 0;
...
<?php } ?>
    ...
    function getExemption() {
        $.post(host + 'FeesJH/view_exemption_jh', {doc_token_no: $('#token_no').val()}, function (exmData) {
    ...
    function update_sd() {
        $.ajax({
            type: 'post',
            url: host + 'CitizenentryFeesJH/update_sd_jh',
            data: $("#frmid").serialize(),
            success: function (result) {
                if (result == 1) {
                    alert('SD Updated');
...
//                $('#ulb_type_id').val('');
    ...
    function getStmpCacl() {
        $.post(host + 'FeesJH/view_sd_calc_jh', {doc_token_no: $('#token_no').val(), fee_type_id: 1, 'lang': $('#lang').val()}, function (SDCalc) {
            $("#SDCalcDetail").html("");
    ...
    function getArticleFeeItems() {
        $.post(host + 'FeesJH/get_article_fee_rule_item_input_jh', {feerule_id: $("#feeRule_id option:selected").val(), 'doc_token_no': $('#token_no').val(), 'lang': $('#lang').val()}, function (itemdata) {
            $("#article_items").html("");
    ...
    function caculateMV() {
        if ($('#feeRule_id').val()) {
            if ($('#frmid')[0].checkValidity()) {
                $.ajax({
                    type: 'post',
                    url: host + 'FeesJH/calculate_mv_jh',
                    data: $("#frmid").serialize(),
                    success: function (result) {
...
//------------------------------------------------------------------------------------------------------------------------------------------------
    function getamount(val_id, ulb_id, land_type_id, property_id) {
        ...
        $.post(host + 'CitizenentryFeesJH/get_valuation_amt_sd_jh', {val_id: val_id, property_id: property_id}, function (data) {
            if (data === 'a') {
...
<?php if ($area_flag == 'Y') { ?>
        $.post(host + 'CitizenentryFeesJH/get_area_jh', {val_id: val_id}, function (data1) {
...
//        });
    ...
    function getAdjDocExsAmt() {
        ...
        if (doc_no != '' && doc_date != '') {
            $.post(host + 'CitizenentryFeesJH/get_adj_doc_exess_amt_jh', {'adj_doc_no': doc_no, 'adj_doc_date': doc_date}, function (exsAmt) {
                $('#adj_amt').val(exsAmt);
                $('#adj_amt').prop('readonly', false);
                $('#adj_amt').attr('max', exsAmt);
            });
            $.post(host + 'CitizenentryFeesJH/get_adj_doc_exess_amt_detail_jh', {'adj_doc_no': doc_no, 'adj_doc_date': doc_date}, function (oldDocDetail) {
                                ...
                                <div class="col-sm-12">
                                        ...
                                        if ($property_list) {
                                            foreach ($property_list as $key => $property) {
                                                ...
                                                echo $this->Form->input('exm_prop_id', array('type' => 'hidden', 'id' => 'exm_prop_id', 'value' => $property[0]['property_id']));
                                            }
                                        }
                                        ?>
                                </div>
