...
<!--<script>
    ...
    function OnLoad() {
        ...
        control.makeTransliteratable(["usage_sub_catg_desc_ll"]);
        var keyVal = 32; // Space key
        $("#usage_sub_catg_desc_en").on('keydown', function (event) {
            if (event.keyCode === 32) {
                var engText = $("#usage_sub_catg_desc_en").val().trim() + " ";
                var engTextArray = engText.split(" ");
                $("#usage_sub_catg_desc_ll").val($("#usage_sub_catg_desc_ll").val() + engTextArray[engTextArray.length - 2]);

                document.getElementById("usage_sub_catg_desc_ll").focus();
                $("#usage_sub_catg_desc_ll").trigger({
                    type: 'keypress', keyCode: keyVal, which: keyVal, charCode: keyVal
                });
            }
        });

        $("#usage_sub_catg_desc_ll").bind("keyup", function (event) {
            setTimeout(function () {
                $("#usage_sub_catg_desc_en").val($("#usage_sub_catg_desc_en").val() + " ");
                document.getElementById("usage_sub_catg_desc_en").focus()
            }, 0);
        });
    } //end onLoad function
...
<script>
    ...
    function formupdate(usage_sub_catg_desc_en, usage_sub_catg_desc_ll, dolr_usage_code, id) {
        document.getElementById("actiontype").value = '1';
...
<?php echo $this->Form->create('Usagesub', array('id' => 'Usagesub', 'autocomplete' => 'off')); ?>
<?php echo $this->Form->input('csrftoken', array('label' => false, 'id' => 'csrftoken', 'type' => 'hidden', 'class' => 'form-control', 'value' => $this->Session->read("csrftoken"))); ?>
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-primary">
            <div class="panel-heading" style="text-align: center"><b><?php echo __('lblUsagesubcategoryhead'); ?></b></div>
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table id="tableUsagesub" class="table table-striped table-bordered table-condensed">  
                        <thead >  
                            <tr>  
                        ...
                        <tbody>
                            <tr>
                                <?php foreach ($Usagesubrecord as $Usagesubrecord1): ?>
                                    ...
                                    <?php if ($this->Session->read("sess_langauge") == 'en') { ?>
                                        <td style="text-align: center;"><?php echo $Usagesubrecord1['Usagesub']['usage_sub_catg_desc_en']; ?></td>
                                        <td style="text-align: center;"><?php echo $Usagesubrecord1['Usagesub']['dolr_usage_code']; ?></td>
                                        <td style="text-align: center;"><?php echo $Usagesubrecord1['Usagesub']['usage_sub_catg_desc_ll']; ?></td>
                                        <td style="text-align: center;">
                                            <button id="btnupdate" name="btnupdate" class="btn btn-default " style="text-align: center;" 
                                                    onclick="javascript: return formupdate(
                                                                                            ('<?php echo $Usagesubrecord1['Usagesub']['usage_sub_catg_desc_en']; ?>'),
                                                                                            ('<?php echo $Usagesubrecord1['Usagesub']['usage_sub_catg_desc_ll']; ?>'),
                                                                                            ('<?php echo $Usagesubrecord1['Usagesub']['dolr_usage_code']; ?>'),
                                                                                            ('<?php echo $Usagesubrecord1['Usagesub']['id']; ?>'));">
                                                <span class="glyphicon glyphicon-pencil"></span>
                                            </button>
                                            <button id="btndelete" name="btndelete" class="btn btn-default " style="text-align: center;" 
                                                    onclick="javascript: return formdelete(('<?php echo $Usagesubrecord1['Usagesub']['id']; ?>'));">
