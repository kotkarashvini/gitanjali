...
if (isset($paymentfields) && !empty($paymentfields)) {
    ?>
    <?php echo $this->Form->create('payment', array('url' => array('controller' => 'Registration', 'action' => 'payment_verification')), array('id' => 'payment', 'class' => 'form-vertical')); ?>   
...
<div class="form-group">
    <label> <?php echo $field['field_name_desc_' . $lang] ?> <span class="star" ><?php echo $field['required_mark']; ?></span></label>
        ...
        if ($field['field_name'] == 'account_head_code') {
            ...
            $drop_option= $this->requestAction(array('controller' => 'Registration', 'action' => 'get_payment_field_values',$field['field_id'] ));  
...
foreach ($feedetails as $fee) {
    ...
    foreach ($modemapping as $accdetails) {
        if($fee[0]['fee_item_id']==$accdetails['items']['fee_item_id']){
           $matchflag=1; 
