        ...
        $('#appointment_date').datepicker({
            ...
            $.post('<?php echo $this->webroot; ?>Citizenentry/check_appointmentdate', {app_date: app_date, csrftoken: csrftoken}, function (data)
            {
                ...
                if (data.trim() == 'a')
                {
                    ...
                    {
                        $.post('<?php echo $this->webroot; ?>Citizenentry/gov_slot_alocation', {office_id: office_id, app_date: app_date, shift_id: shift_id, csrftoken: csrftoken}, function (data)
                        {
            ...
            { 
                ...
                $.post('<?php echo $this->webroot; ?>Citizenentry/check_appointmentdate', {app_date: app_date,csrftoken:csrftoken}, function (data)
                {
                    ...
                    if (data.trim() == 'a')
                    {
                        ...
                        $.post('<?php echo $this->webroot; ?>Citizenentry/check_govmaxappointmentday', {app_date: app_date, office_id: office_id, shift_id: shift_id,csrftoken:csrftoken}, function (data1)
                        {
                            if (data1.trim() != 'b')
                            {
...
<?php if ($submission_flag == 'Y') { ?>
    <?php echo $this->Form->create('govappoinment', array('id' => 'govappoinment')); ?>

    <div class="row">
        <div class="col-lg-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <center><h3 class="box-title headbolder"><?php echo __('lblgovappidetails'); ?></h3><a href="<?php echo $this->webroot;?>helpfiles/citizenentry/tatkalappoinment_<?php echo $doc_lang; ?>.html" class="btn btn-default pull-right " target="_blank"> <?php echo  __('Help');?> <span class="fa fa-question fa-circle-o"></span></a></center>
                </div>
