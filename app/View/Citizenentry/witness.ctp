    ...
    $(document).ready(function () {
        ...
        $.post(host + 'Citizenentry/get_witness_feilds', {csrftoken: csrftoken}, function (fields)
        {
            if (fields) {
                ...
                window.location.href = "<?php echo $this->webroot; ?>Citizenentry/witness/<?php echo $this->Session->read('csrftoken'); ?>";
                            }
                        });
                    });
                    ...
                    {
                        $.post(host + 'Citizenentry/get_witness_feilds', {id: id, csrftoken: csrftoken}, function (data)
                        {
                    ...
                    function formdelete(id) {
                        ...
                        if (result) {
                            $.post('<?php echo $this->webroot; ?>Citizenentry/delete_witness', {id: id, csrftoken: csrftoken}, function (data1)
                            {
...
echo $this->Form->create('witness', array('id' => 'witness', 'class' => 'form-vertical'));
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-12">
                                            <p style="color: red;"><b><?php echo __('lblnote'); ?>1:&nbsp;</b><?php echo __('lblengdatarequired'); ?>
                                                <?php if ($stateid == 3) { ?>
                                                    <b><?php echo __('lblnote'); ?>2:&nbsp;</b><?php echo __('For Witnesses, preference should be given to neighbourhood of the executant'); ?>
                                                <?php } ?>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <?php for ($i = 0; $i < count($witness); $i++) { ?>
                            <tr>
                                ...
                                <td >
                                    ...
                                    <a <?php echo $this->Html->Link($this->Html->tag('span', '', array('class' => 'glyphicon glyphicon-remove')), array('action' => 'delete_witness', $this->Session->read('csrftoken'), $witness[$i][0]['id']), array('escape' => false, 'data-toggle' => 'tooltip', 'title' => __('Delete'), 'class' => "btn btn-default"), array('Are you sure?')); ?></a>
