...
//        alert('hi');
        ...
        $('input:radio[name="data[frm][exemption_flag]"]').change(function () {
            ...
            if ($(this).val() == 'Y') {
                ...
                $.post(host + 'deleteexemption', {}, function (Data) {
                    getExemption();
                    $('#exemption_amt').val(0);
                    $('#exemption_div').hide();
                });
...
//        ($('#sd_adj_flag').val() == 'Y') ? $('#sd_adj_div').show() : $('#sd_adj_div').hide();
        ...
        if ($("#feeRule_id option:selected").val()) {
            ...
            $.post('<?php echo $this->webroot; ?>Citizenentry/get_exmption_rule', {fee_rule_id: fee_rule_id}, function (data)
            {
...
<?php } ?>
        ...
        $('#exemption_feeRule_id').change(function () {
            ...
            if ($(this).val()) {
                $.post(host + 'getFeeRuleInputs', {feerule_id: $(this).val(), exmption: 'Y'}, function (ExmItemData) {
        ...
        $('#btnNext').click(function () {
            ...
            $.post(host + 'getFeesCalcFlag', {doc_token_no: $('#token_no').val()}, function (data) {
                var prop_count = 0;
...
//------------------------------------------------------------------------------------------------------------------------------------------------
    function getamount(val_id, ulb_id, land_type_id, property_id) {
        ...
        $.post(host + 'Citizenentry/get_valuation_amt_sd', {val_id: val_id, property_id: property_id}, function (data) {
            if (data === 'a') {
...
<?php if ($area_flag == 'Y') { ?>
            $.post(host + 'Citizenentry/get_area', {val_id: val_id}, function (data1) {
...
<?php if ($gov_body_flag == 'Y') { ?>
            $.post(host + 'Citizenentry/get_gov_body', {val_id: val_id}, function (data2) {

                if (data2 == 'a') {
                    alert('Please valuate again');
                    return false;
                } else {
                    $("input[name='data[frm][FCQ]']").val(Math.round(data2));
                }
            });
<?php } ?>

<?php if ($mutation_fee_flag == 'Y') { ?>
            $.post(host + 'Citizenentry/get_party_share', {property_id: property_id}, function (data1) {
                if (data1 == 'a') {
                    alert('Please valuate again');
                    return false;
                } else {
                    $("input[name='data[frm][FCY]']").val(Math.round(data1));
