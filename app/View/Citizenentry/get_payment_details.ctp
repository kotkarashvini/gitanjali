...
<?php if (isset($paymentfields) && !empty($paymentfields)) {
    ?>
    <div class="col-md-12">
        <?php echo $this->Form->create('payment', array('url' => array('controller' => 'Citizenentry', 'action' => 'payment',$this->Session->read('csrftoken'))), array('id' => 'payment', 'class' => 'form-vertical')); ?>   
        ...
        foreach ($paymentfields as $field) {
            ...
            if (isset($payment) && !empty($payment)) {
                $upadteflag = 1;
                $payment1[$field['field_name']] = $payment[0]['CitizenPaymentEntry'][$field['field_name']];
                $payment1['payment_mode_id'] = $payment[0]['CitizenPaymentEntry']['payment_mode_id'];
            } else {
        ...
        if ($upadteflag == 1) {
            echo $this->Form->input('payment_id', array('label' => false, 'id' => 'id', 'class' => 'form-control input-sm', 'type' => 'hidden', 'value' => $payment[0]['CitizenPaymentEntry']['payment_id']));
        }
        ?>
        ...
        <div class="row center">
            <div class="col-lg-12">
                <div class="form-group">
                    ...
                    <?php echo $this->Html->link('New Entry', array('controller' => 'Citizenentry', 'action' => 'payment',$this->Session->read('csrftoken')), array('class' => 'btn btn-danger', 'escape' => false));
                    ?>
                </div>
            </div>
        </div>       
