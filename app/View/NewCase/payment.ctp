    ...
    $(document).ready(function () {
        ...
        $('#paymentmode_selection1').change(function (e) {
            ...
            if (mode === '')
            {
                ...
                $.post('<?php echo $this->webroot; ?>NewCase/get_ccms_payment_details', {mode: mode, csrftoken:<?php echo $this->Session->read('csrftoken'); ?>}, function (data)
                {
...
<?php } ?>
                    $('#bank_id').change(function (e) {
                        ...
                        if (bank !== '')
                        {
                            $.getJSON('<?php echo $this->webroot; ?>Citizenentry/get_bank_branch', {bank: bank, csrftoken:<?php echo $this->Session->read('csrftoken'); ?>}, function (data)
                            {
                                ...
                                $('#branch_id').change(function (e) {
                                    ...
                                    if (branch !== '')
                                    {
                                        $.getJSON('<?php echo $this->webroot; ?>Citizenentry/get_bank_branch_code', {branch: branch, csrftoken:<?php echo $this->Session->read('csrftoken'); ?>}, function (data)
                                        {
                                            $.each(data, function (index, val) {
                                                $("#ifsc_code").val(val);
                                            });
...
?>
    ...
    function edit_payment(mode, id)
    {
        if (mode === '' && id === '')
        {
            ...
            $.post('<?php echo $this->webroot; ?>NewCase/get_ccms_payment_details', {mode: mode, id: id, paymenttoken: 'certifiedcopy', csrftoken:<?php echo $this->Session->read('csrftoken'); ?>}, function (data)
            {
                $('#paydetails').html(data);
...
<!--                        <label for="" class="col-sm-2 control-label">Selected Case:-<span style="color: #ff0000"></span></label>    
                        ...
                        <label for="" class="col-sm-2 control-label">Final Judgement<span style="color: #ff0000"></span></label>    
                        <div class="col-sm-2">
                            : <?php
                        ...
                        <label for="" class="col-sm-2 control-label">Judgement Date<span style="color: #ff0000"></span></label>    
                        <div class="col-sm-2">
...
//                                    pr($feedetails);exit;
                                                ...
                                                if ($fee[0]['account_head_code'] == $paydetails['account_head_code']) {
                                                    ...
                                                    <td width="15%" class="bg-info">    
                                                        <div class="btn-group">                                         
                                                            <?php
                                                            $water_mark = "";
                                                            // echo $this->Html->link('PDF', array('controller' => 'Reports', 'action' => 'doc_payment_receipt', $paydetails['pay_id'], $water_mark), array('class' => 'btn btn-danger', 'escape' => false));

                                                            if ($status == 99) { //and $documents[0][0][$funflag] == 'N'
                                                                ?>
                                                                <input type="button" class="btn btn-danger" value="Edit"    onclick="edit_payment('<?php echo $paydetails['payment_mode_id']; ?>', '<?php echo $paydetails['pay_id']; ?>');"> 
                                                                <a   class="btn btn-danger" href="<?php echo $this->webroot; ?>NewCase/certified_copy_payment_entry/<?php echo $paydetails['pay_id']; ?>">Delete</a>
                                                            <?php } ?>
                                                        </div>   
                                                    </td>       
                                                    <?php
                        ...
                        <div class="panel-footer center"> 
                            ...
                            echo $this->Form->create('payment_accept');
