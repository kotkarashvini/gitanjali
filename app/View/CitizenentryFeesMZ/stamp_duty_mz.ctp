...
//        alert('hi');
        ...
        $('input:radio[name="data[frm][exemption_flag]"]').change(function () {
            ...
            if ($(this).val() == 'Y') {
                ...
                $.post(host + 'FeesMZ/delete_fee_exemption_mz', {}, function (Data) {
                    getExemption();
                    $('#exemption_amt').val(0);
                    $('#exemption_div').hide();
                });
...
//        ($('#sd_adj_flag').val() == 'Y') ? $('#sd_adj_div').show() : $('#sd_adj_div').hide();
        ...
        if ($("#feeRule_id option:selected").val()) {
            ...
            $.post('<?php echo $this->webroot; ?>CitizenentryFeesMZ/get_exmption_rule_mz', {fee_rule_id: fee_rule_id}, function (data)
            {
...
<?php } ?>
        ...
        $('#exemption_feeRule_id').change(function () {
            ...
            if ($(this).val()) {
                $.post(host + 'FeesMZ/get_article_fee_rule_item_input_mz', {feerule_id: $(this).val(), exmption: 'Y'}, function (ExmItemData) {
        ...
        $("#btnCalculateExemption").click(function () {
            var exm_arg = {doc_token_no: $('#token_no').val(), exm: 'Y', fee_rule_id: $('#exemption_feeRule_id').val(), FAS: $('#onlineSDAmount').val(), FAG: $('#FAG').val(), FAF: $('#FAF').val(), FBG: $('#FBG').val(), FBF: $('#FBF').val(), property_id: $('#exm_prop_id').val()};
            $.ajax({
                type: 'post',
                url: host + 'FeesMZ/calculate_fees_mz',
                data: exm_arg,
        ...
        $('#btnNext').click(function () {
            ...
            $.post(host + 'CitizenentryFeesMZ/get_fees_falc_ids_mz', {doc_token_no: $('#token_no').val()}, function (data) {
                var prop_count = 0;
...
<?php } ?>
    ...
    function getExemption() {
        $.post(host + 'FeesMZ/view_exemption_mz', {doc_token_no: $('#token_no').val()}, function (exmData) {
    ...
    function update_sd() {
        $.ajax({
            type: 'post',
            url: host + 'CitizenentryFeesMZ/update_sd_mz',
            data: $("#frmid").serialize(),
            success: function (result) {
                if (result == 1) {
                    alert('SD Updated');
...
//                $('#ulb_type_id').val('');
    ...
    function getStmpCacl() {
        $.post(host + 'FeesMZ/view_sd_calc_mz', {doc_token_no: $('#token_no').val(), fee_type_id: 1, 'lang': $('#lang').val()}, function (SDCalc) {
            $("#SDCalcDetail").html("");
    ...
    function getArticleFeeItems() {
        $.post(host + 'FeesMZ/get_article_fee_rule_item_input_mz', {feerule_id: $("#feeRule_id option:selected").val(), 'doc_token_no': $('#token_no').val(), 'lang': $('#lang').val()}, function (itemdata) {
            $("#article_items").html("");
    ...
    function caculateMV() {
        if ($('#feeRule_id').val()) {
            if ($('#frmid')[0].checkValidity()) {
                $.ajax({
                    type: 'post',
                    url: host + 'FeesMZ/calculate_mv_mz',
                    data: $("#frmid").serialize(),
                    success: function (result) {
...
//------------------------------------------------------------------------------------------------------------------------------------------------
    function getamount(val_id, ulb_id, land_type_id, property_id) {
        ...
        $.post(host + 'CitizenentryFeesMZ/get_valuation_amt_sd_mz', {val_id: val_id, property_id: property_id}, function (data) {
            if (data === 'a') {
...
<?php if ($area_flag == 'Y') { ?>
            $.post(host + 'CitizenentryFeesMZ/get_area_mz', {val_id: val_id}, function (data1) {
...
//        });
    ...
    function getAdjDocExsAmt() {
        ...
        if (doc_no != '' && doc_date != '') {
            $.post(host + 'CitizenentryFeesMZ/get_adj_doc_exess_amt_mz', {'adj_doc_no': doc_no, 'adj_doc_date': doc_date}, function (exsAmt) {
                $('#adj_amt').val(exsAmt);
                $('#adj_amt').prop('readonly', false);
                $('#adj_amt').attr('max', exsAmt);
            });
            $.post(host + 'CitizenentryFeesMZ/get_adj_doc_exess_amt_detail_mz', {'adj_doc_no': doc_no, 'adj_doc_date': doc_date}, function (oldDocDetail) {
...
<?php if ($property_list) { ?>
                    <div class="row">
                        <div class="form-group">
                            ...
                            <?php if($article_id==91) { ?>
                            <?php if($title_id!=56){ ?>
                             <label for="amt" class="col-sm-2 control-label"><?php echo __('lblconsamt_'.$article_id.'_'.$title_id); ?></label> 
                            <?php } ?>
                            <?php }else {?>
