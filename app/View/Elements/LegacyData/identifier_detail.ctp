<script>
    $(document).ready(function () {
        $('#tbl_legacy_identifiers').dataTable();
    });
</script>
<div id ='legacy_identifier_detail'>
    <table id="tbl_legacy_identifiers"  style="width:100%" class="table table-striped table-bordered table-condensed">
        <thead>
            <tr>
                <th class="center"><?= __('Doc. Reg. No.'); ?></th>
                <th class="center"><?= __('Identifier Name'); ?></th>
   ...
   if(!empty($identifierDetail)) {
        foreach ($identifierDetail as $record) {
        ?>
            <tr>
