...
//        alert('hi');
        ...
        $('input:radio[name="data[frm][exemption_flag]"]').change(function () {
            ...
            if ($(this).val() == 'Y') {
                ...
                $.post(host + 'FeesHP/delete_fee_exemption_hp', {}, function (Data) {
                    getExemption();
                    $('#exemption_amt').val(0);
                    $('#exemption_div').hide();
                });
...
//        ($('#sd_adj_flag').val() == 'Y') ? $('#sd_adj_div').show() : $('#sd_adj_div').hide();
        ...
        if ($("#feeRule_id option:selected").val()) {
            ...
            $.post('<?php echo $this->webroot; ?>CitizenentryFeesHP/get_exmption_rule_hp', {fee_rule_id: fee_rule_id}, function (data)
            {
...
//        getExemption();
        ...
        $("#btnCalcAndSave").click(function (e) {
            if ($('#feeRule_id').val()) {
<?php if (!empty($property_list) && ($prop_flag != 0)) { ?>
...
<?php } ?>
        ...
        $('#exemption_feeRule_id').change(function () {
            ...
            if ($(this).val()) {
                $.post(host + 'FeesHP/get_article_fee_rule_item_input_hp', {feerule_id: $(this).val(), exmption: 'Y'}, function (ExmItemData) {
        ...
        $("#btnCalculateExemption").click(function () {
            var exm_arg = {doc_token_no: $('#token_no').val(), exm: 'Y', fee_rule_id: $('#exemption_feeRule_id').val(), FAS: $('#onlineSDAmount').val(), FAG: $('#FAG').val(), FAF: $('#FAF').val(), FBG: $('#FBG').val(), FBF: $('#FBF').val(), FBG: $('#FBG').val(), FHJ: $('#FHJ').val(), FHI: $('#FHI').val(),FBD: $('#FBD').val(), property_id: $('#exm_prop_id').val()};
            $.ajax({
                type: 'post',
                url: host + 'FeesHP/calculate_fees_hp',
                data: exm_arg,
        ...
        $('#btnNext').click(function () {
            ...
            $.post(host + 'CitizenentryFeesHP/get_fees_falc_ids_hp', {doc_token_no: $('#token_no').val()}, function (data) {
                var prop_count = 0;
...
<?php } ?>
    ...
    function getExemption() {
        $.post(host + 'FeesHP/view_exemption_hp', {doc_token_no: $('#token_no').val()}, function (exmData) {
    ...
    function update_sd() {
        $.ajax({
            type: 'post',
            url: host + 'CitizenentryFeesHP/update_sd_hp',
            data: $("#frmid").serialize(),
            success: function (result) {
                if (result == 1) {
                    alert('SD Updated');
...
//                $('#ulb_type_id').val('');
    ...
    function getStmpCacl() {
        $.post(host + 'FeesHP/view_sd_calc_hp', {doc_token_no: $('#token_no').val(), fee_type_id: 1, 'lang': $('#lang').val()}, function (SDCalc) {
            $("#SDCalcDetail").html("");
    ...
    function getArticleFeeItems() {
        $.post(host + 'FeesHP/get_article_fee_rule_item_input_hp', {feerule_id: $("#feeRule_id option:selected").val(), 'doc_token_no': $('#token_no').val(), 'lang': $('#lang').val()}, function (itemdata) {
            $("#article_items").html("");
...
//------------------------------------------------------------------------------------------------------------------------------------------------
    function getamount(val_id, ulb_id, land_type_id, property_id) {
        $('#ulb_type_id').val(ulb_id);
        $('#land_type_id').val(land_type_id);
        $.post(host + 'CitizenentryFeesHP/get_valuation_amt_sd_hp', {val_id: val_id, property_id: property_id}, function (data) {
            if (data === 'a') {
                ...
                $.each(JSON.parse(data), function (i, amt)
                {
                    ...
                    if (i === 'cons') {
                        if (amt > 0) {
                            ...
                            $("input[name='data[frm][cons_amt]']").val(0);
                        }
                    }
                    if (i === 'party') {
                        $('#party_count').val(Math.round(amt));
                        $("input[name='data[frm][FCX]']").val(Math.round(amt));
                    }
                });
            }
        });
...
//        });
    ...
    function getAdjDocExsAmt() {
        ...
        if (doc_no != '' && doc_date != '') {
            $.post(host + 'CitizenentryFeesHP/get_adj_doc_exess_amt_hp', {'adj_doc_no': doc_no, 'adj_doc_date': doc_date}, function (exsAmt) {
                $('#adj_amt').val(exsAmt);
                $('#adj_amt').prop('readonly', false);
                $('#adj_amt').attr('max', exsAmt);
            });
            $.post(host + 'CitizenentryFeesHP/get_adj_doc_exess_amt_detail_hp', {'adj_doc_no': doc_no, 'adj_doc_date': doc_date}, function (oldDocDetail) {
                ...
                <div class="table-responsive" style="height:20vh; overflow-y: scroll;">
                    <table class="table table-striped table-bordered table-hover" id="tblproperty_list">
                        ...
                        <tbody>
                            ...
                            <tr id="<?php echo $property[0]['property_id']; ?>">
                                <td class="tblbigdata">                                          
                                    <input type="radio" value="<?php echo $property[0]['property_id']; ?>" name="property_list" onclick="javascript: return getamount('<?php echo $property[0]['val_id']; ?>', '<?php echo $property[0]['ulb_type_id']; ?>', '<?php echo $property[0]['developed_land_types_id']; ?>', '<?php echo $property[0]['property_id']; ?>');">   
                                </td>
                                <td class="tblbigdata"><?php echo $property[0]['property_id']; ?></td>
                                <td class="tblbigdata">
                                                        <?php
...
<?php if ($property_list) { 
    ...
    if($considaration_amount_flag=='Y'){
        if($title_id==157 || $title_id==158 || $title_id==159 || $title_id==160){ ?>
            <div class="row">
                    <div class="form-group">
                        <div class="col-sm-2"></div>
<!--                            <label for="amt" class="col-sm-2 control-label"><?php echo __('lblconsiderationamount'); ?></label> -->
                        <div class="col-sm-2">
    <?php echo $this->Form->input('cons_amt1', array('type' => 'hidden', 'label' => false, 'id' => 'cons_amt1', 'value'=>'0','class' => 'form-control input-sm')); ?>                             
                                ...
                                <div class="col-sm-12">
                                        ...
                                        if ($property_list) {
                                            foreach ($property_list as $key => $property) {
                                                $prop_id = $property[0]['property_id'];
                                                echo $this->Form->input('exm_prop_id', array('type' => 'hidden', 'id' => 'exm_prop_id', 'value' => $prop_id));
                                            }
                                        }
                                        ?>
                                </div>
