...
//      alert('All Items,Subrules from Rule Id:"' + Base64.decode(copy_id) + '"  are Copied');
    ...
    //-------------------------------------------------------Paste Rule---------------------------------------------------------------
    function pasteRule(to_id) { alert(to_id);
       // alert($('#copy_rule_id').val());
        if ($('#copy_rule_id').val() != '' && $('#copy_rule_id').val()) {
//        var toid= decode64(to_id);
//        alert(toid);
            if (confirm('Are You Sure? Do you want to copy all items,conditions and formula from Rule No. ' +$('#copy_rule_id').val() + ' to ' + to_id)) {
//    alert(encode64($('#copy_rule_id').val()));
    ...
    <div class="col-lg-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <center><h3 class="box-title" style="font-weight: bolder"><?php echo __('lblfeerule'); ?></h3></center>
...
//                            pr($erd);exit;
                            ...
                            . $this->Form->button('<i class = "fa fa-files-o"></i>', array('title' => 'Copy', 'class' => "copy", 'onclick' => "javascript: return copyRule('" . $erd['fee_rule_id'] . "')"))
                            . $this->Form->button('<i class = "fa fa-clipboard"></i>', array('title' => 'Paste', 'class' => "", 'onclick' => "javascript: return pasteRule('" . $erd['fee_rule_id'] . "')"))
                            . "</td>"
                            . "</tr>";
                        }
                        ?>
