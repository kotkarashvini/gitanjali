 ...
 * @author nic
 */
class MZSearchController extends AppController {
                ...
                if ($frmdate <= $todate && $todate >= $new_date && $frmdate >= $new_date) {
                    ...
                    if ($this->validationError($errarr)) {
                        if ($this->Searcher->save($this->request->data['searchindex'])) {
                            ...
                            $this->redirect(array('controller' => 'MZSearch', 'action' => 'searchdefault', $district_id, $frmdate, $todate));
...
//            $officeid = $this->Auth->User("office_id");
            ...
            $imagedata = "img/state_logos_img/MZ_logo.png";
...
//            $pages = $value + $total_party + 4;
            ...
            $downloaddeed = "";
            if ($searcherdata['payment_status'] == 'Y') {
                $downloaddeed .= "<a href='" . $this->webroot . "MZSearch/downloadfile/" . $scan_upload['scan_upload']['scan_name'] . "/Scanning/" . $_POST['token_no'] . "/C' class='btn btn-primary'>Download Deed</a>";
            } else {
                $downloaddeed .= "<a href='" . $this->webroot . "MZWebService/gras_payment_entry' target='_blank' class='btn btn-primary'>Make Payment</a>";
                $downloaddeed .= "<a href='" . $this->webroot . "MZSearch/payment_verification' target='_blank' class='btn btn-primary'>Verify Payment</a>";
            }
