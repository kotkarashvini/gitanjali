 ...
 * @author nic
 */
class TRSearchController extends AppController {
                ...
                if ($frmdate <= $todate && $todate >= $new_date && $frmdate >= $new_date) {
                    ...
                    if ($this->validationError($errarr)) {
                        if ($this->Searcher->save($this->request->data['searchindex'])) {
                            ...
                            $this->redirect(array('controller' => 'TRSearch', 'action' => 'searchdefault', $district_id, $frmdate, $todate));
//                                $this->set('documentrecord', $this->document->find('all'));
                    ...
                    $this->redirect(array('controller' => 'TRLegacySearch', 'action' => 'searchdefault', $district_id, $frmdate, $todate));
...
//                            pr($condition);
                            $searchbyparty = $this->Searcher->query("SELECT distinct p.party_id, p.token_no,p.party_id,year.year, aps.doc_reg_no as final_doc_reg_no ,p.party_full_name_en,p.father_full_name_en,p.address_en, p.address2_en
                                                ...
                                                join ngdrstab_trn_serial_numbers year on year.token_no=aps.token_no
                                                where $condition");
...
//                            $condition = "art.book_number='BK1' and aps.final_stamp_flag='Y' and DATE(aps.doc_reg_date)>= '$startdate' and DATE(aps.doc_reg_date)<= '$enddate' and p.taluka_id=$taluka_id and p.village_id=$village_id";
                            ...
                            $searchbyproperty = $this->Searcher->query("SELECT  distinct p.property_id, p.token_no,year.year,
                                    ...
                                    LEFT JOIN ngdrstab_trn_volume_number_page_number_entry volno ON volno.token_no=aps.token_no
                                    join ngdrstab_mst_office ofs on ofs.office_id=aps.office_id
                                    LEFT join ngdrstab_trn_serial_numbers year on year.token_no=aps.token_no
...
//                pr($villagename);exit;
    ...
    public function fee_receipt() {
        try {
            ...
            $submitdata = $this->office->query("select doc_reg_no,doc_reg_date from ngdrstab_trn_application_submitted where token_no=?", array($_POST['token_no']));
            ...
            $year = $this->office->query("select year from ngdrstab_trn_serial_numbers where token_no=?", array($_POST['token_no']));
            if (!empty($year)) {
                $year = $year[0][0]['year'];
            } else {
                $year = null;
...
//            }
            ...
            $downloaddeed = "";
            if ($searcherdata['payment_status'] == 'Y') {
                $downloaddeed .= "<a href='" . $this->webroot . "TRSearch/downloadfile/" . $scan_upload['scan_upload']['scan_name'] . "/Scanning/" . $_POST['token_no'] . "/C' class='btn btn-primary'>Download Deed</a>";
            } else {
                ...
                $downloaddeed .= "<a href='" . $this->webroot . "TRSearch/payment_verification' target='_blank' class='btn btn-primary'>Verify Payment</a>";
            }
...
//                    ."</thead>"
    ...
    public function esearhpartydata($token_no, $srno) {
        ...
        if (!empty($result[0][0]['link_doc_no'])) {
            ...
            $token = $this->office->query("select token_no from ngdrstab_trn_application_submitted where doc_reg_no ='$link_doc_no'");
