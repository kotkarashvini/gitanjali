...
App::import('Controller', 'FeesMZ'); // mention at top
...
class CitizenentryFeesMZController extends FeesMZController {
...
//        $this->Auth->allow('welcomenote', 'login', 'add', 'Disclaimer', 'index', 'index1', 'index2', 'registration', 'checkuser', 'viewsingle', 'ViewRegisteruser', 'get_district_name', 'get_captcha', 'aboutus', 'contactus', 'insertuser', 'checkorg', 'sponsordetail_pdf', 'checkcaptcha', 'checkemail', 'send_sms', 'empregistration');
    ...
    function consideration_amount_mz() {
        try {
...
// flag update
                    ...
                    $this->redirect('consideration_amount_mz');
                }
            }
            $this->set_csrf_token();
        } catch (Exception $ex) {
...
///-------*--------*-*-------------------*-----------------------*----------Stamp Duty Related----------*--------------------*---------*---------------------
    public function stamp_duty_mz($csrftoken = NULL) {
        // load Model
        try {
            ...
            //load Model
            array_map(array($this, 'loadModel'), array('valuation_details', 'party_entry', 'investment_details', 'articledescdetails', 'article', 'article_screen_mapping', 'exemption_article_rules', 'property_details_entry', 'article_fee_rule', 'stamp_duty', 'stamp_duty_adjustment', 'conf_reg_bool_info', 'office', 'conf_article_feerule_items','genernalinfoentry'));
            ...
            if ($this->Session->read("article_id") == 32) {
                ...
                if (empty($property_list)) {
                    ...
                    return $this->redirect(array('action' => 'consideration_amount_mz', $this->Session->read('csrftoken')));
                }
            } else {
                $property_list = $this->property_details_entry->get_property_list($doc_lang, $citizen_token_no, $user_id);
            }
            ...
            $title_id_result=$this->genernalinfoentry->query("select title_id from ngdrstab_trn_generalinformation where token_no=$citizen_token_no");
            if(!empty($title_id_result))
            {
                $title_id=$title_id_result[0][0]['title_id'];
            }
            ...
            $this->set(compact('lang', 'article_id', 'title_id' ,'investor_flag', 'feeRuleList', 'exemption_flag', 'exemption_rule', 'citizen_token_no', 'article_rule', 'property_pattern', 'property_list', 'sd_adj_flag', 'adjustmentdocument_flags', 'delay_flag', 'office'));

            //validations kalyani
            $this->loadModel('NGDRSErrorCode');
            $result_codes = $this->NGDRSErrorCode->find("all");
            $this->set('result_codes', $result_codes);
            $name_format = $this->get_name_format_mz();
...
//                        $this->stamp_duty_adjustment->saveField('old_data_flag', $frm['old_data_flag']);
                                ...
                                $this->update_sd_mz($frm);
                                if ($frm['online_adj_doc_no'] && $frm['online_adj_doc_date'] && $frm['old_data_flag'] === 'Y') {
                                    $adjustable_amount = $this->get_adj_doc_exess_amt($frm['online_adj_doc_no'], $frm['online_adj_doc_date']);
                                    if ($frm['online_adj_amt'] < $adjustable_amount) {
                                        $sd_update_result = $this->update_sd_mz($frm);
                                    } else {
                                        $sd_update_result = 0;
                                        $this->Session->setFlash('Adustment amount shound not be greater than ' . $adjustable_amount);
                                    }
...
//------------------------------------------------------------------------------------------------------------------------------------------------
                if ($sd_update_result == 1) {
                    ...
                    $this->redirect(array('action' => 'stamp_duty_mz', $this->Session->read('csrftoken')));
                } else {
...
//                }
    ...
    //-------*---------------------------------------------------------------------------------------------------------------------------------------------------
    public function update_sd_mz($formData = NULL) {
        try {
    ...
    public function get_name_format_mz() {
     ...
     function get_adj_doc_exess_amt_mz($adj_doc_no = NULL, $adj_doc_date = NULL) {
        try {
    ...
    function get_adj_doc_exess_amt_detail_mz($adj_doc_no = NULL, $adj_doc_date = NULL) {
        try {
//            $this->autoRender = TRUE;
    ...
    function get_valuation_amt_mz($val_id = NULL) {
        try {
    ...
    function get_valuation_amt_sd_mz($val_id = NULL, $property_id = NULL) {
        try {
            if (isset($_POST['csrftoken'])) {
                $this->check_csrf_token_withoutset($_POST['csrftoken']);
            }
    ...
    function get_area_mz($val_id = NULL) {
        try {
