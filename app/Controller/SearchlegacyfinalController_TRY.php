    ...
    function upload_excel_to_tbl_data()
    {
      ...
      //  PR('HIII3');
        if ($this->request->is('post')) 
        {
            ...
            pr($this->request->data['upload_excel_to_tbl']['batch_no']);
...
//                        $fieldlist['doc_scan_date']['text'] = '';
                        ...
                        PR($this->getvalidationruleset($fieldlist));
                        ...
                        for ($m = 1; $m <= $sheet0_count - 1; $m++) {
                            ...
                            //pr("select * from ngdrstab_trn_tmp_legacy_generalinformation where doc_reg_no=" . "'" . $doc_reg_no . "' and $m=$m" . "and doc_processing_year=" . $doc_process_yr . " and state_id=" . $state_id . "and office_id=" . $office_id . "and book_code=" . "'" . $book_code . "'");
...
//                            }
                        ...
                        if ($geninfocount == 0) {
                          ...
                          PR('SAVE TO TEMP');
                            ...
                            $info_property_serial_no = $this->save_tmp_property_details();
                           
                           $SAVEDATA= $this->importFromTmpToTrnTables($info_arr);
    ...
    public function save_tmp_general_info($sheet0_count, $excel, $info_arr)
    {
        ...
        for ($m = 1; $m <= $sheet0_count - 1; $m++) {
            try {
              ...
              $FINAL=$this->Leg_generalinformation->query("insert into ngdrstab_trn_legacy_generalinformation(reference_sr_no,doc_reg_no,book_code,doc_reg_date,doc_processing_year,state_id,district_id,taluka_id,office_id,doc_entered_office,local_language_id,article_id,exec_date,presentation_no,presentation_dt,year_for_token,doc_type,reference_no,user_id,req_ip,last_status_id,last_status_date,batch_id,batch_no,is_doc_scanned,doc_scan_date) values($reference_sr_no,'$doc_reg_no','$book_code','$doc_reg_date',$doc_process_yr,$state_id,$district_id,$taluka_id,$office_id,$doc_entered_office,$lang,$article,'$exc_date','$presentation_no','$presentation_dt',$year_for_token,$doc_type,'$reference_no',$user_id,'$req_ip',$last_status_id,'$last_status_date',$batch_id,$batch_no,'$is_doc_scanned','$doc_scan_date')");
