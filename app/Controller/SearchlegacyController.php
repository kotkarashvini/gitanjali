...
//    public $components = array('Security', 'RequestHandler', 'Captcha', 'Cookie');
    ...
    public function beforeFilter() {
        ...
        $this->Auth->allow('upload_excel_to_tbl', 'get_party_type');
    ...
    function upload_excel_to_tbl() {
        try {
            ...
            if ($this->request->is('post')) {
                ...
                //echo '<br>'.$user_id= $this->Session->read("citizen_user_id");
                //exit;
                if ($this->request->data['upload_excel_to_tbl']['upload_file']['error'] == 0) {
                    ...
                    // $excel->read('D:\NGDRS_Upload_ga\Dharbandora_2011_legacy_data_format_with_10_sample_data.xls');
                    // $excel->read('D:\NGDRS_Upload_ga\salcete_10_sample_data.xls');
                    $excel->read($to_read);
                    // print number of rows, columns and sheets
                    echo "Number of sheets: " . sizeof($excel->sheets) . "\n";
                    /* for ($x=0; $x<sizeof($excel->sheets); $x++) {
                      echo "<br><br>Number of rows in sheet " . ($x+1) . ": " . $excel->sheets[$x]["numRows"] . "\n";
                      echo "<br>Number of columns in sheet " . ($x+1) . ": " . $excel->sheets[$x]["numCols"] . "\n";
                      } */
//-----------------------VALIDATION SHEET 1----------------------------------
                    $fieldlist = array();
                    $fieldlist2 = array();
...
//-----------------------VALIDATION  SHEET 1----------------------------------
                        ...
                        $fieldlist['doc_reg_date']['text'] = 'is_required'; //DATE FORMAT
                        ...
                        $fieldlist['office_name_en']['text'] = 'is_alphaspacedashslash'; //"Valid entry A-Z,a-z,0-9,-,.,/,( ,) and space allowed"
...
//-----------------------VALIDATION  SHEET 1----------------------------------
                        ...
                        //echo '<br>'.$doc_reg_date=$excel->sheets[0]['cells'][$i][5];
                        ...
                        //echo '  '.$doc_reg_date=$excel->sheets[0]['cellsInfo'][$i][5]['raw'];
                        // echo '  '.$doc_reg_date=date('Y-m-d', strtotime(str_replace('/', '-', trim($excel->sheets[0]['cells'][$i][5]))));
                        $doc_processing_year = $excel->sheets[0]['cells'][$i][6];
                        //  pr($doc_processing_year);exit;
                        ...
                        //-----------------------VALIDATION  SHEET 1----------------------------------
                        $uniqueno = $excel->sheets[0]['cells'][$i][16];
                        //-----------------------VALIDATION  SHEET 1----------------------------------
...
//-----------------------VALIDATION  SHEET 1----------------------------------
                        ...
                        if ($this->ValidationError($errarr)) {
                            $inst = $this->LegacyDataMain->query("insert into ngdrstab_trn_legacy_data_main(state_id,state_name_en,reference_sr_no,doc_reg_no,doc_reg_date,doc_processing_year,article_id,article_desc_en,office_district_id,office_district_name_en,office_taluka_id,office_taluka_name_en,office_id,office_name_en,user_id,creation_date,doc_file_nm,uniqueno) values($state_id,'$state_name_en',$reference_sr_no,'$doc_reg_no','$doc_reg_date','$doc_processing_year',$article_id,'$article_desc_en',$office_district_id,'$office_district_name_en',$office_taluka_id,'$office_taluka_name_en',$office_id,'$office_name_en',$user_id,'$creation_date','$doc_file_nm','$uniqueno')");
                        } else {
                            $errmsg = NULL;
                            ...
                            // echo '<br>err:'.$errmsg;
                            $inst_invalid = $this->LegacyDataMainInvalidData->query("insert into ngdrstab_trn_legacy_data_main_invalid_data(state_id,state_name_en,reference_sr_no,doc_reg_no,doc_reg_date,doc_processing_year,article_id,article_desc_en,office_district_id,office_district_name_en,office_taluka_id,office_taluka_name_en,office_id,office_name_en,user_id,creation_date,doc_file_nm,uniqueno,invalid_msg) values($state_id,'$state_name_en',$reference_sr_no,'$doc_reg_no','$doc_reg_date','$doc_processing_year',$article_id,'$article_desc_en',$office_district_id,'$office_district_name_en',$office_taluka_id,'$office_taluka_name_en',$office_id,'$office_name_en',$user_id,'$creation_date','$doc_file_nm',$uniqueno,'$errmsg')");
...
////-----------------------VALIDATION  SHEET 5----------------------------------
                        ...
                        $regdate = $excel->sheets[4]['cells'][$m][4];
                        $unixTimestamp = ($regdate - 25569) * 86400;
                        $doc_reg_date = date('Y-m-d', $unixTimestamp);
...
////-----------------------VALIDATION  SHEET 5----------------------------------                        
                       ...
                       // $data['unixTimestamp'] = $unixTimestamp;
                        ...
                        if ($this->ValidationError($errarr5)) {
                            $inst5=$this->LegacyDataParty->query("insert into ngdrstab_trn_legacy_data_party(state_id,reference_sr_no,doc_reg_no,doc_reg_date,doc_processing_year,party_sr_no,party_type_desc_en,category_name_en,party_full_name_en,father_full_name_en,gender,uniqueno) values($state_id,$reference_sr_no,'$doc_reg_no','$doc_reg_date','$doc_processing_year',$party_sr_no,'$party_type_desc_en','$category_name_en','$party_full_name_en','$father_full_name_en','$gender','$uniqueno')");
                        } else {
                            $errmsg5 = NULL;
...
////-----------------------VALIDATION  SHEET 6----------------------------------
                       ...
                       // $data['unixTimestamp'] = $unixTimestamp;
                        ...
                        if ($this->ValidationError($errarr6)) {
                             $inst6=$this->LegacyDataPayment->query("insert into ngdrstab_trn_legacy_data_payment(state_id,reference_sr_no,doc_reg_no,doc_reg_date,doc_processing_year,payment_sr_no,fee_item_desc_en,final_value,uniqueno) values($state_id,$reference_sr_no,'$doc_reg_no','$doc_reg_date','$doc_processing_year',$payment_sr_no,'$fee_item_desc_en',$final_value,'$uniqueno')");
                        } else {
                            $errmsg6 = NULL;
                            ...
                            //echo '<br>err:'.$errmsg;
                            $inst_invalid = $this->LegacyDataPaymentInvalidData->query("insert into ngdrstab_trn_legacy_data_payment_invalid_data(state_id,reference_sr_no,doc_reg_no,doc_reg_date,doc_processing_year,payment_sr_no,fee_item_desc_en,final_value,uniqueno,invalid_msg) values($state_id,$reference_sr_no,'$doc_reg_no','$doc_reg_date','$doc_processing_year',$payment_sr_no,'$fee_item_desc_en',$final_value,'$uniqueno','$errmsg6')");
                            // $this->Session->setFlash('Invalid data is getting uploaded in record no ' . $unique_record . '(' . $errmsg . ')');
                            //$this->redirect(array('action' => 'upload_excel_to_tbl', $this->Session->read('csrftoken')));
                        }
                    }
