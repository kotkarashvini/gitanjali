 ...
 * @author nic
 */
class GASearchController extends AppController {
...
//                $frmdate = "'" . date('Y-m-d', strtotime($frmdate)) . "'";
//                $todate = "'" . date('Y-m-d', strtotime($todate)) . "'";
                $datedata = $this->Searcher->query("select * from ngdrstab_conf_date where state_id=31");
                ...
                if ($frmdate <= $todate && $todate >= $new_date && $frmdate >= $new_date) {
                    ...
                    if ($this->validationError($errarr)) {
                        if ($this->Searcher->save($this->request->data['searchindex'])) {
                            ...
                            $this->redirect(array('controller' => 'GASearch', 'action' => 'searchdefault', $district_id, $frmdate, $todate));
...
//            }
            ...
            $downloaddeed = "";
            if ($searcherdata['payment_status'] == 'Y') {
                $downloaddeed .= "<a href='" . $this->webroot . "GASearch/downloadfile/" . $scan_upload['scan_upload']['scan_name'] . "/Scanning/" . $_POST['token_no'] . "/C' class='btn btn-primary'>Download Deed</a>";
            } else {
