...
class FeesMZController extends AppController {
...
//            'article_item_link_not_sd', 'get_articledependentfeild');
    ...
    public function get_article_fee_rule_item_input_mz($fee_rule_id = NULL, $doc_token_no = NULL, $lang = 'en') {
        try {
...
//                ));
                ...
                foreach ($itemdata as $FeeItem) {
                    ...
                    if ($item_code == 'FCY') {
                        $party1 = $this->party_entry->query("select count(party_id) from ngdrstab_trn_party_entry_new where party_type_id=19  and token_no=$doc_token_no");
...
//                    }
                ...
                $this->set(compact('party_count_new', 'total_party', 'area', 'gender_new', 'flag', 'gov_body_result', 'party_count', 'itemvalue', 'itemdata', 'items_list', 'form_name', 'lang', 'optional_fees'));
...
//------------------***------------------------------------------Fee Calulation for stamp duty,Registration,Handeling Charges and Others-----------------------------------------------      
    public function calculate_mv_mz($frm = NULL) {
        try {
    ...
    public function calculate_fees_mz($itemValues = NULL, $returnCalc = 'N') {
        /*
...
//            } else 
            ...
            $feeRule = $this->get_calc_fee_rule_list_mz($frm);
//            pr($feeRule);exit;
            if ($feeRule) {
                $success_flag = 'N';
                $feeCalcDetail = array();
...
//                        $subruleData = $this->article_fee_subrule->find('all', array('fields' => array('article_fee_subrule.*'), 'conditions' => $subrule_conditions, 'joins' => array(array('table' => 'ngdrstab_mst_article_fee_items', 'alias' => 'item', 'conditions' => array('item.fee_item_id=article_fee_subrule.fee_output_item_id'))), 'order' => 'article_fee_subrule.fee_output_item_order'));
                        ...
                        if ($subruleData) {
                            foreach ($subruleData as $fsrl) {
                                ...
                                $tmpfeeCalcDetail['calc_detail'] = $this->check_fee_condition_mz($articleItems, $frm, $fsrl);
...
//                            array_push($feeCalcDetail, $tmpfeeCalcDetail);
                ...
                if ($success_flag == 'Y') {
                    ...
                    if ($this->fees_calculation->save($frm)) {
                        ...
                        if ($this->save_fee_calculation_detail_mz($fees_cal_id, $frm, $feeCalcDetail, $articleItems)) {
                            if ($frm['article_id'] == 9998 && isset($frm['token_no']) && $frm['token_no']) {
                                $this->update_fee_exemption_mz($frm['token_no']);
                                ...
                                return $this->view_fee_calculation_mz($fees_cal_id);
                            }
                        }
                    } else {
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
    public function get_calc_fee_rule_list_mz($frm) {
        try {
            $this->autoRender = FALSE;
            $feeRuleList = array();
            $ruleCondition = array();
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
    public function save_fee_calculation_detail_mz($fees_cal_id, $frm, $feeCalcDetail, $articleItems) {
        try {
            $this->autoRender = FALSE;
            ...
            if ($this->fees_calculation_detail->saveAll($fee_calc_detail_final)) {
                return $this->view_fee_calculation_mz($frm['fee_calc_id']);
            } else {
...
//-------------------------------------------------------------------Check Fee Rule Conditions---------------------------------------------------------------------------------
    public function check_fee_condition_mz($articleItems, $frm, $fsrl) {
        try {
// pr($frm);exit;
...
//                }
            ...
            if ($output_item) {
                ...
                if ($fsrl['fee_rule_cond1']) {
                    if (eval("return(" . $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_cond1']) . ");")) {
                        $calculations = $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula1']);
                    } else if ($fsrl['fee_rule_cond2']) {
                        if (eval("return(" . $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_cond2']) . ");")) {
                            $calculations = $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula2']);
                        } else if ($fsrl['fee_rule_cond3']) {
                            if (eval("return(" . $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_cond3']) . ");")) {
                                $calculations = $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula3']);
                            } else if ($fsrl['fee_rule_cond4']) {
                                if (eval("return(" . $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_cond4']) . ");")) {
                                    $calculations = $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula4']);
                                } else if ($fsrl['fee_rule_cond5']) {
                                    $calculations = (eval("return(" . $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_cond5']) . ");")) ? $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula5']) : -1;
                                } else {//if condition 5 not available
                                    $calculations = ($fsrl['fee_rule_formula5']) ? $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula5']) : -1;
                                }
                            } else {//if condition 4  not available
                                $calculations = ($fsrl['fee_rule_formula4']) ? $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula4']) : -1;
                            }
                        } else {// if condition 3  not available
                            $calculations = ($fsrl['fee_rule_formula3']) ? $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula3']) : -1;
                        }
                    } else {//if condition 2 not available
                        $calculations = ($fsrl['fee_rule_formula2']) ? $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula2']) : -1;
                    }
                } else {//condition 1  not Available              
                    $calculations = ($fsrl['fee_rule_formula1']) ? $this->add_value_to_formula_mz($articleItems, $frm, $fsrl['fee_rule_formula1']) : -1;
                }
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
    public function add_value_to_formula_mz($itemlist, $value, $formula) {// for replacing item code with value in formula
        try {
            $this->autoRender = FALSE;
//-----------------------for calculated Market Value for multiple Ietm-------------------
            ...
            if ($mv_item_flag == 'N' && $itemlist) {
                ...
                $value['OMV'] = $this->calculate_mv_mz($value);
                if (isset($value['cons_amt'])) {
...
//----------------------------------------------------------------------------------------
    ...
    public function get_certificate_fees_mz() {
        try {
            $this->autoRender = FALSE;
...
//------------------***--------------------------------view Calculated Report-----------------------------------------------------------------------------------------
    public function view_fee_calculation_mz($fee_calc_id = NULL, $view_flag = NULL) {
        try {
            $this->autoRender = FALSE;
...
//                    $out_vwReport.="</table>";
    ...
    public function view_sd_calc_mz($doc_token_no = NULL, $fee_type_id = 2, $sess_lang = 'en') {
        try {
            /*
...
//-------------------------------------------------------------------Update Fee Exemption --------------------------------------------------------------------------------
    public function update_fee_exemption_mz($doc_token_no) {
        try {
...
//------------------***-------------view Exemption--------------------------------------------------------------
    ...
    public function delete_fee_exemption_mz() {
        try {
    ...
    public function view_exemption_mz($doc_token_no = NULL, $sess_lang = NULL) {
        try {
            $this->autoRender = FALSE;
...
//-----------------------------for Adding additional Items to article (MRM-Citizon Entry) by Madhuri R------------------------------------------------------------------------
    function article_item_link_not_sd_mz() {
        try {
...
//-----------------------------------Get Article Dependant Input Intems--by Madhuri R----------------------------
    public function get_article_dependent_feild_mz() {
        try {
