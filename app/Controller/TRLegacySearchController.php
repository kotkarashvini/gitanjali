 ...
 * @author nic
 */
class TRLegacySearchController extends AppController {
...
//                        $condition = "art.book_number='BK1' and aps.final_stamp_flag='Y' and DATE(aps.doc_reg_date)>= '$startdate' and DATE(aps.doc_reg_date)<= '$enddate'";
                        $condition = " aps.final_stamp_flag='Y' and DATE(aps.doc_reg_date)>= '$startdate' and DATE(aps.doc_reg_date)<= '$enddate'";
                        ...
                        if (!empty($party_name) && !empty($article_id)) {
                            ...
                            $condition .=  " and upper(p.party_full_name_en) like '%$party_name%'";
                            $condition .=  " and gen.article_id=$article_id";
                            ...
                            $searchbyparty = $this->Searcher->query("SELECT distinct p.party_id, p.token_no,p.party_id,year.year, aps.doc_reg_no as final_doc_reg_no ,p.party_full_name_en,p.father_full_name_en,p.address_en, p.address2_en
                                                ...
                                                ,pt.party_type_desc_en,ofs.office_name_en
                                                 FROM ngdrstab_trn_legacy_party_entry_new p
                                                join ngdrstab_mst_party_type pt on pt.party_type_id=p.party_type_id 
                                                left join ngdrstab_trn_legacy_application_submitted aps on aps.token_no=p.token_no
                                                join ngdrstab_trn_legacy_generalinformation gen on gen.token_no=aps.token_no
                                                ...
                                                join ngdrstab_trn_legacy_serial_numbers_final year on year.token_no=aps.token_no
                                                where $condition");
...
//                            $enddate = $toyear . '-12-31';
                            $startdate = date('Y-m-d', strtotime($fromyear));
                            $enddate = date('Y-m-d', strtotime($toyear));
//                            $condition = "art.book_number='BK1' and aps.final_stamp_flag='Y' and DATE(aps.doc_reg_date)>= '$startdate' and DATE(aps.doc_reg_date)<= '$enddate' and p.taluka_id=$taluka_id and p.village_id=$village_id";
                            $condition = " aps.final_stamp_flag='Y' and DATE(aps.doc_reg_date)>= '$startdate' and DATE(aps.doc_reg_date)<= '$enddate' and p.taluka_id=$taluka_id and p.village_id=$village_id";
                            ...
                            $searchbyproperty = $this->Searcher->query("SELECT  distinct p.property_id, p.token_no,year.year,
                                    aps.doc_reg_no as final_doc_reg_no,dist.district_name_en,art.article_desc_en,ofs.office_name_en, 
                                    art.book_number,volno.volume_number,volno.page_number_start,volno.page_number_end
                                    FROM ngdrstab_trn_legacy_property_details_entry p
                                    join ngdrstab_trn_legacy_application_submitted aps on aps.token_no=p.token_no
                                    join ngdrstab_trn_legacy_generalinformation gen on gen.token_no=aps.token_no
                                    join ngdrstab_mst_article art on art.article_id=gen.article_id
                                    join ngdrstab_conf_admblock3_district dist on dist.district_id=gen.district_id
                                    LEFT JOIN ngdrstab_trn_volume_number_page_number_entry volno ON volno.token_no=aps.token_no
                                    join ngdrstab_mst_office ofs on ofs.office_id=aps.office_id
                                    LEFT join ngdrstab_trn_legacy_serial_numbers_final year on year.token_no=aps.token_no
                                    join ngdrstab_trn_legacy_parameter pr on pr.token_id=p.token_no
                                    where $condition");
                    ...
                    if ($_POST['action'] == 3) {
                        ...
                        if (!empty($deed_no)) {
//                            $condition = "art.book_number='BK1' and aps.final_stamp_flag='Y' and aps.doc_reg_no='$deed_no'";
                            $condition = "aps.final_stamp_flag='Y' and aps.doc_reg_no='$deed_no'";
...
//                pr($villagename);exit;
    ...
    public function propertydata($token_no) {
        ...
        $lang = $this->Session->read("sess_langauge");
        $getdata = $this->Searcher->query("select village.village_name_$lang,village.jh_taluka_code, property.val_id  from ngdrstab_trn_legacy_property_details_entry as property
        ...
        $searchbydeed = $this->Searcher->query("select concat(mp.eri_attribute_name,' - ',paramter_value)  from ngdrstab_trn_legacy_parameter as tp 
    ...
    public function fee_receipt() {
        try {
            ...
            $submitdata = $this->office->query("select doc_reg_no,doc_reg_date from ngdrstab_trn_legacy_application_submitted where token_no=?", array($_POST['token_no']));
            ...
            $year = $this->office->query("select year from ngdrstab_trn_legacy_serial_numbers_final where token_no=?", array($_POST['token_no']));
            if (!empty($year)) {
                $year = $year[0][0]['year'];
            } else {
                $year = null;
...
//            }
            ...
            $itemvalue = $this->article_fee_item_list->query("select no_of_pages from ngdrstab_trn_legacy_generalinformation where token_no=$doc_token_no");
            ...
            $data['final_doc_reg_no'] = $submitdata['doc_reg_no'];
            $data['final_stamp_date'] = $submitdata['doc_reg_date'];
            $data['pages'] = $pages;
            $data['fee'] = $fee;
            $data['amount'] = $amount;
            ...
            $downloaddeed = "";
            if ($searcherdata['payment_status'] == 'Y') {
                $downloaddeed .= "<a href='" . $this->webroot . "TRLegacySearch/downloadfile/" . $scan_upload['scan_upload']['scan_name'] . "/Scanning/" . $_POST['token_no'] . "/C' class='btn btn-primary'>Download Deed</a>";
            } else {
                ...
                $downloaddeed .= "<a href='" . $this->webroot . "TRLegacySearch/payment_verification' target='_blank' class='btn btn-primary'>Verify Payment</a>";
            }
...
//               $this->create_pdf($html_design, 'Yearly Report', 'A4-P', '');
    ...
    public function rptnonemcomb_copy() {
        try {
            ...
            $searchbydeed = $this->office->query("select concat(mp.eri_attribute_name,' - ',paramter_value)  from ngdrstab_trn_legacy_parameter as tp 
                                                JOIN  ngdrstab_mst_attribute_parameter  as mp ON mp.attribute_id=tp.paramter_id
                                                where tp.token_id=$token_no
                                                order by mp.eri_attribute_name");

            $vt = $this->office->query("select gen.token_no,t.taluka_name_en,d.district_name_en,v.village_name_en  from ngdrstab_trn_legacy_property_details_entry gen 
...
//                    ."</thead>"
    ...
    public function esearhpartydata($token_no, $srno) {
        ...
        $result = $this->office->query("SELECT p.token_no,p.party_id,aps.doc_reg_no as final_doc_reg_no,p.party_full_name_en,p.father_full_name_en,p.address_en, p.address2_en
        ...
        $html_design .= "</td><td style='text-align:center;'><b>" . $result[0][0]['doc_reg_no'] . "</b></td>"
        ...
        if (!empty($result[0][0]['link_doc_no'])) {
            ...
            $token = $this->office->query("select token_no from ngdrstab_trn_legacy_application_submitted where doc_reg_no ='$link_doc_no'");
    ...
    public function propertydatanew($token_no) {
        ...
        $lang = $this->Session->read("sess_langauge");
        $getdata1 = $this->Searcher->query("select property.property_id, village.village_name_$lang,village.jh_taluka_code, property.val_id  from ngdrstab_trn_legacy_property_details_entry as property
                                                JOIN  ngdrstab_conf_admblock7_village_mapping  as village ON village.village_id=property.village_id
                                                where property.token_no=$token_no");
        $propdata = array();
        $villagename = null;
        ...
        foreach ($getdata1 as $key => $getdata) {
            ...
            $searchbydeed = $this->Searcher->query("SELECT 
                                                concat(mparam.eri_attribute_name_$lang,' - ',param.paramter_value)
                                                FROM ngdrstab_trn_legacy_property_details_entry as prop
                                                JOIN ngdrstab_trn_legacy_parameter as param ON param.property_id=prop.property_id
    ...
    public function rptdetails() {
        try {
            ...
            $result = $this->office->query("SELECT p.token_no,p.party_id,aps.doc_reg_no,aps.doc_reg_no as final_doc_reg_no,p.party_full_name_en,p.father_full_name_en,p.address_en, p.address2_en
                                    ,art.book_number,volno.volume_number,volno.page_number_start,volno.page_number_end, gen.link_doc_no
                                    ,pt.party_type_desc_en,ofs.office_name_en,aps.doc_reg_date,art.article_desc_en,aps.id,year.year
                                     FROM ngdrstab_trn_legacy_party_entry_new p
                                    left outer join ngdrstab_mst_party_type pt on pt.party_type_id=p.party_type_id 
            ...
            if (!empty($result)) {
                ...
                $html_design = "<html><body><style>td{padding:5px;}  </style>"
                        ...
                        . "<tr><td><b>Deed No. : " . $result[0][0]['doc_reg_no'] . "</b></td></tr>"
                ...
                if ($flag == 'P') {
                    ...
                    $html_design .="<tr><td><b>Deed No : " . $result[0][0]['doc_reg_no'] . "</b></td></tr>"
    ...
    public function payment_verification() {
        try {
            ...
            if ($this->request->is('post')) {
                ...
                $article_id = $this->office->query("select article_id from ngdrstab_trn_legacy_generalinformation where token_no=?", array($searcher[0][0]['token_no']));
