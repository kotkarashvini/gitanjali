 ...
 * @author nic
 */
class JHSearchController extends AppController {
    ...
    public function beforeFilter() {
        ...
        $this->Auth->allow('searchindex', 'downloadfile1');
    }
                ...
                if ($frmdate <= $todate && $todate >= $new_date && $frmdate >= $new_date) {
                    ...
                    if ($this->validationError($errarr)) {
                        if ($this->Searcher->save($this->request->data['searchindex'])) {
                            ...
                            $this->redirect(array('controller' => 'JHSearch', 'action' => 'searchdefault', $district_id, $frmdate, $todate));
//                                $this->set('documentrecord', $this->document->find('all'));
                    ...
                    /* $this->request->data['searchindex']['req_ip'] = $_SERVER['REMOTE_ADDR'];
                      ...
                      $this->Session->write("application_id", $application_id[0]['Searcher']['application_id']);
                      $this->Session->setFlash(__('The search application submitted.'));
                      $this->redirect(array('controller' => 'SearchModule', 'action' => 'search_data_module', $district_id, $frmdate, $todate));
                      //                                $this->set('documentrecord', $this->document->find('all'));
                      } else {
                      $this->Session->setFlash(__('Something went wrong ... !!!!'));
                      }
                    ...
                    pr(" Pleae select from and to date either greater than $old_date or more than $new_date...!!!");
                }
            }
            $this->set_csrf_token();
        } catch (Exception $ex) {
...
//                            $condition = "art.book_number='BK1' and aps.final_stamp_flag='Y' and DATE(aps.final_stamp_date)>= '$startdate' and DATE(aps.final_stamp_date)<= '$enddate' and p.taluka_id=$taluka_id and p.village_id=$village_id";
                            ...
                            if (!empty($this->request->data['searchdefault']['plot_no'])) {
                                ...
                                $condition = $condition . " and pr1.paramter_id=206 and pr1.paramter_value='$plot_no'";
                            }
...
//pr($searchbyproperty);exit;
                            $searchbyproperty = $this->Searcher->query("SELECT  distinct p.property_id, p.token_no,year.year,
                                    ...
                                    join ngdrstab_trn_parameter pr1 on pr1.token_id=p.token_no
                                    where $condition");
//pr($searchbyproperty);exit;
...
//            $officeid = $this->Auth->User("office_id");
            $officeid = $this->office->query("select office_id from ngdrstab_trn_application_submitted where token_no=?", array($_POST['token_no']));
            $officeid = $officeid[0][0];
            ...
            $vt = $this->office->query("select gen.token_no,t.taluka_name_en,d.district_name_en,v.village_name_en
                                            ,gen.district_id, gen.taluka_id, gen.village_id
                                            from ngdrstab_trn_property_details_entry gen 
...
//===================chagned on 11-06-2019 for tanvir request============================================================================================================
            
            $paramvalue = $this->office->query("select paramter_id , paramter_value FROM ngdrstab_trn_parameter
                                                where token_id=$token_no and paramter_id IN(206,205)
                                                order by paramter_id");
//            pr($paramvalue);exit;
            $khatano=$paramvalue[0][0]['paramter_value'];
            $plotno=$paramvalue[1][0]['paramter_value'];
            $villageid = $vt[0][0]['village_id'];
            $talukaid = $vt[0][0]['taluka_id'];
            $alltokens = $this->office->query("SELECT  distinct p.token_no, aps.final_stamp_date
                                    FROM ngdrstab_trn_property_details_entry p
                                    join ngdrstab_trn_application_submitted aps on aps.token_no=p.token_no
                                    join ngdrstab_trn_generalinformation gen on gen.token_no=aps.token_no
                                    join ngdrstab_trn_parameter pr on pr.token_id=p.token_no
                                    join ngdrstab_trn_parameter pr1 on pr1.token_id=p.token_no
                                    where aps.final_stamp_flag='Y' and DATE(aps.final_stamp_date)>= '$fromdate' and DATE(aps.final_stamp_date)<= '$todate'
                                    and p.taluka_id=$talukaid and p.village_id=$villageid
                                    and pr.paramter_id=205 and pr.paramter_value='$khatano'
                                    and pr1.paramter_id=206 and pr1.paramter_value='$plotno'
                                    order by aps.final_stamp_date");
            $srno=1;
            foreach ($alltokens as $tokens) {
                $token_no = $tokens[0]['token_no'];
                $html_design .= $this->esearhpartydata($token_no, $srno++);
            }
 //=====================================================================================================================================================================           
...
//            pr($html_design);exit;
    ...
    public function esearhpartydata($token_no, $srno) {
        ...
        $result = $this->office->query("SELECT p.token_no,p.party_id,aps.final_doc_reg_no,p.party_full_name_en,p.father_full_name_en,p.address_en, p.address2_en
                                    ...
                                    LEFT JOIN ngdrstab_trn_volume_number_page_number_entry volno ON volno.token_no=aps.token_no
                                    join ngdrstab_mst_office ofs on ofs.office_id=aps.office_id
                                    where p.token_no=$token_no and p.display_flag='Y'
                                    order by p.party_id");
    ...
    public function esearhpartydata1($token_no, $srno) {
...
//            $officeid = $this->Auth->User("office_id");
            ...
            $result = $this->office->query("SELECT p.token_no,p.party_id,aps.final_doc_reg_no,p.party_full_name_en,p.father_full_name_en,p.address_en, p.address2_en
                                    ...
                                    LEFT JOIN ngdrstab_trn_volume_number_page_number_entry volno ON volno.token_no=aps.token_no
                                    join ngdrstab_mst_office ofs on ofs.office_id=aps.office_id
                                    where p.token_no=$token_no and p.display_flag='Y' order by p.party_id");
...
//                            pr('/usr/java/jdk1.8.0_131/bin/java -jar ' . $jarpath . ' ' . $source . ' ' . $destination . ' ' . $flag);
    ...
    function downloadfile1() {
        try {
            ...
            $folder = '/Documents/Dhanbad/222/43/20190000008994/Scanning/';
            $token = 20190000008994;
            $file = '20190000008994.pdf';
            $flag = 'C';
            $path = $this->file_config->find('first', array('fields' => array('filepath')));
            $jarpath = $path['file_config']['filepath'] . "jar_files/Watermarkstring.jar";
            $tempfolder = "Temp/";
...
//                            pr($source);
//                            pr($destination);
//                            pr('/usr/java/jdk1.8.0_131/bin/java -jar ' . $jarpath . ' ' . $source . ' ' . $destination . ' ' . $flag);exit;
            $message = exec('/usr/java/jdk1.8.0_131/bin/java -jar ' . $jarpath . ' ' . $source . ' ' . $destination . ' ' . $flag, $result);
            $this->any_download_file($destination, $file);
        } catch (Exception $e) {
            $this->redirect(array('controller' => 'Errors', 'action' => 'error404'));
...
//            pr($path);pr($file);exit;
    ...
    public function fee_receipt() {
        try {
            ...
            $lang = $this->Session->read("sess_langauge");
            $submitdata = $this->office->query("select final_doc_reg_no,final_stamp_date,office_id from ngdrstab_trn_application_submitted where token_no=?", array($_POST['token_no']));
            $submitdata = $submitdata[0][0];
            $officeid = $submitdata['office_id'];
            $officename = $this->office->query("select office_name_$lang from ngdrstab_mst_office where office_id = $officeid");
            $officename = $officename[0][0]['office_name_' . $lang];
            $application_id = $this->Session->read("application_id");
            $searcherdata = $this->office->query("select * from ngdrstab_trn_searcher_datails where application_id = $application_id");
            $searcherdata = $searcherdata[0][0];
            $check = $this->office->query("select * from ngdrstab_trn_payment_details where token_no=? and application_id=?", array($searcherdata['token_no'],$application_id));
            if($searcherdata['payment_status'] == 'Y' and $check == null){
               $downloaddeed .= "<a href='" . $this->webroot . "JHSearch/searchindex'  class='btn btn-primary'>Click for New LOGIN</a>";
                $html_design='<H2>You have already Donwnloaded Certified Copy..!!! You have to login with new application for another Certified Copy....!!!</H2>';
...
//            }
            $fname = $this->filepath($_POST['token_no']);
            $pages = exec('/usr/bin/pdfinfo ' . $fname . ' | awk \'/Pages/ {print $2}\'', $output);
...
//            $pages = $value + $total_party + 4;
            ...
            $downloaddeed = "";
            if ($searcherdata['payment_status'] == 'Y') {
                $downloaddeed .= "<a href='" . $this->webroot . "JHSearch/downloadfile/" . $scan_upload['scan_upload']['scan_name'] . "/Scanning/" . $_POST['token_no'] . "/C' class='btn btn-primary'>Download Deed</a>";
            } else {
                $downloaddeed .= "<a href='" . $this->webroot . "JHWebService/gras_payment_entry' target='_blank' class='btn btn-primary'>Make Payment</a>";
                $downloaddeed .= "<a href='" . $this->webroot . "JHSearch/payment_verification' target='_blank' class='btn btn-primary'>Verify Payment</a>";
            }
...
//               $this->create_pdf($html_design, 'Yearly Report', 'A4-P', '');
    ...
    public function payment_verification() {
        try {
            ...
            if ($this->request->is('post')) {
                pr($this->request->data);
                $data['bank_trn_id'] = $this->request->data['payment']['bank_trn_id'];
                $data['f_vender_name'] = $this->request->data['payment']['f_vender_name'];
                $data['payment_mode_id'] = $this->request->data['payment']['payment_mode_id'];
                $check = $this->office->query("select * from ngdrstab_trn_online_payment_received where bank_trn_id = '" . $data['bank_trn_id'] . "'");
//                pr($check);exit;
                if ($check == null) {
                    $application_id = $this->Session->read("application_id");
                    ...
                    $response = $serviceobj->GrasVerification($data, $extrafields);

                    if (empty($response['Error'])) {
                        $pdata['payment_mode_id'] = $data['payment_mode_id'];
                        $pdata['cin_no'] = $response['CIN'];
                        $pdata['pdate'] = date('Y/m/d H:i:s');
                        $pdata['pamount'] = $response['TXN_AMOUNT'];
                        $pdata['bank_trn_id'] = $response['DEPTTRANID'];
                        $pdata['f_vender_name'] = $data['f_vender_name'];
                        $pdata['payment_date'] = $response['TXN_DATE'];
                        $pdata['state_id'] = $this->Auth->User('state_id');
                        $pdata['org_user_id'] = $this->Auth->User('user_id');
                        $pdata['req_ip'] = $_SERVER['REMOTE_ADDR'];
                        $pdata['token_no'] = $response['DEPOSITERID'];
                        ...
                        $pdata['payee_fname_en'] = $response['DEPOSITERNAME'];
                        $pdata['grn_no'] = $response['GRN'];
        ...
        try {
            ...
            if (!is_null($token)) {
                ...
                if (!empty($office)) {
                    if (isset($office['district']['district_name_en']) || !is_null($office['district']['district_name_en'])) {
                        if (isset($office['office']['taluka_id']) || !is_null($office['office']['taluka_id'])) {
                            $scan_upload = $this->getscandata($token);
                            $file = $scan_upload['scan_upload']['scan_name'];
                            $folder = "Scanning";
                            $folder = 'Documents/' . $office['district']['district_name_en'] . "/" . $office['office']['taluka_id'] . "/" . $office['office']['office_id'] . "/" . $token . '/' . $folder . '/';
                            $path = $this->file_config->find('first', array('fields' => array('filepath')));
                            $source = $path['file_config']['filepath'] . $folder . $file;
	...
	public function certcopydownload() {
        try {
            array_map([$this, 'loadModel'], ['Searcher']);
            $fieldlist = array();
            $fieldlist['final_doc_reg_no']['text'] = 'is_alphanumspacedashdotslash,is_required';
            $fieldlist['application_id']['text'] = 'is_required,is_digit';
            $this->set('fieldlist', $fieldlist);
            $this->set('result_codes', $this->getvalidationruleset($fieldlist));
            if ($this->request->is('post')) {
                ...
                $errarr = $this->validatedata($this->request->data, $fieldlist);
                    ...
                    $final_doc_reg_no = $this->request->data['final_doc_reg_no'];
                    $application_id = $this->request->data['application_id'];
                    if ($this->ValidationError($errarr)) {
                    $result = $this->Searcher->query("Select * from ngdrstab_trn_searcher_datails where application_id = $application_id and final_doc_reg_no='$final_doc_reg_no'");
                    if (!empty($result)) {
                        if ($result[0][0]['payment_status'] == 'Y') {
                            $token_no = $result[0][0]['token_no'];
                            $created = $result[0][0]['created'];
                            $created = new DateTime($created);
                            $current = new DateTime();
                            $diff = $created->diff($current);
                            $days = $diff->d;
                            $maxdays = $this->Searcher->query("Select info_value from ngdrstab_conf_reg_bool_info where reginfo_id=168");
//                 pr($maxdays);exit;
                            $maxdays = $maxdays[0][0]['info_value'];
                            if ($days < $maxdays) {
                                if ($result[0][0]['download_count'] < 4) {
                                    $count = $result[0][0]['download_count'];
                                    $count++;
                                    $this->Searcher->query("update ngdrstab_trn_searcher_datails set  download_count = $count where application_id = $application_id");
                                    $scan_upload = $this->getscandata($token_no);
                                    $this->downloadfile($scan_upload['scan_upload']['scan_name'], 'Scanning', $token_no, 'C');
                                } else {
                                    $this->Session->setFlash(__('You have downloaded Certified copy already 3 times... !!!!'));
                                }
                            } else {
                                $this->Session->setFlash(__('You can Download Certified copy within 90 days from Payment date... !!!!'));
                            }
                        } else {
                            $this->Session->setFlash(__('Payment is not done for this application... !!!!'));
                        }
                    } else {
                        $this->Session->setFlash(__('Data not found for this Registration Number and Application Number ... !!!!'));
                    }
                }
            }
        } catch (Exception $ex) {
