...
class FeesHPController extends AppController {
...
//            'update_fee_exemption_hp', 'view_exemption',
...
//            $token_no = isset($this->request->data['token_no']) ? $this->request->data['token_no'] : NULL; //18-8-2017s
            if ($fee_rule_id && is_numeric($fee_rule_id)) {
                ...
                /* --------------------------------------------------------------- */
                $itemdata = ClassRegistry::init('conf_article_feerule_items')->find('all', array('fields' => array('DISTINCT item.fee_item_id', 'item.fee_param_code', 'value.articledepfield_value', 'item.fee_item_desc_' . $lang, 'item.list_flag', 'is_hidden'), 'order' => 'item.fee_item_desc_' . $lang, 'conditions' => $condition,
                    'joins' => array(
...
//            return 'error while getting fee rule inputs';
    ...
    public function get_article_fee_rule_item_input_hp($fee_rule_id = NULL, $doc_token_no = NULL, $lang = 'en') {
        try {
            array_map([$this, 'loadModel'], ['conf_reg_bool_info', 'party_entry', 'article_fee_subrule', 'article_fee_item_list', 'property_details_entry', 'valuation_details', 'fee_item_val']);
            $fee_rule_id = isset($this->request->data['feerule_id']) ? $this->request->data['feerule_id'] : $fee_rule_id;
            $exmption = isset($this->request->data['exmption']) ? 'Y' : 'N';
            $this->set('exmption', $exmption);
...
//            $token_no = isset($this->request->data['token_no']) ? $this->request->data['token_no'] : NULL; //18-8-2017s
            if ($fee_rule_id && is_numeric($fee_rule_id)) {
                ...
                /* --------------------------------------------------------------- */
                $itemdata = ClassRegistry::init('conf_article_feerule_items')->find('all', array('fields' => array('DISTINCT item.fee_item_id', 'item.fee_param_code', 'value.articledepfield_value', 'item.fee_item_desc_' . $lang,'item.fee_item_desc_ll4', 'item.list_flag', 'item.display_flag', 'item.display_order', 'is_hidden'), 'order' => array('item.display_flag', 'item.display_order'), 'conditions' => $condition,
                    'joins' => array(
                        array('table' => 'ngdrstab_mst_article_fee_items', 'alias' => 'item', 'conditions' => array('item.fee_item_id=conf_article_feerule_items.fee_item_id')),
                        array('table' => 'ngdrstab_trn_articledepfields', 'alias' => 'value', 'type' => 'left', 'conditions' => array("value.articledepfield_id=conf_article_feerule_items.fee_param_code  and  value.article_id=conf_article_feerule_items.article_id " . $doc_token_no_cond))
                    )
...
//                pr($itemdata);
                ...
                foreach ($itemdata as $FeeItem) {
                    ...
                    $tokenno = $this->property_details_entry->find('all', array('fields' => array('val_id'), 'conditions' => array('token_no' => $doc_token_no)));
                    $rule_new = array();
...
//------------------***------------------------------------------Fee Calulation for stamp duty,Registration,Handeling Charges and Others-----------------------------------------------      
    public function calculate_mv_hp($frm = NULL) {
        try {
...
//            } else 
            ...
            $feeRule = $this->get_calc_fee_rule_list_hp($frm);
            if ($feeRule) {
                $success_flag = 'N';
                $feeCalcDetail = array();
                    ...
                    if ($subruleFlag['article_fee_rule']['sub_fee_rule_flag'] == 'Y') {//if have subrule
                        ...
                        if ($subruleData) {
                            foreach ($subruleData as $fsrl) {
                                ...
                                $tmpfeeCalcDetail['calc_detail'] = $this->check_fee_condition_hp($articleItems, $frm, $fsrl);
...
//                            $tmpfeeCalcDetail['calc_detail'] = $this->check_fee_condition_hp($articleItems, $frm, $ruleData);
...
//                            array_push($feeCalcDetail, $tmpfeeCalcDetail);
                ...
                if ($success_flag == 'Y') {
                    ...
                    if ($this->fees_calculation->save($frm)) {
                        ...
                        if ($this->save_fee_calculation_detail_hp($fees_cal_id, $frm, $feeCalcDetail, $articleItems)) {
                            if ($frm['article_id'] == 9998 && isset($frm['token_no']) && $frm['token_no']) {
                                $this->update_fee_exemption_hp($frm['token_no']);
                                ...
                                return $this->view_fee_calculation_hp($fees_cal_id);
                            }
                        }
                    } else {
...
//                            array_push($feeCalcDetail, $tmpfeeCalcDetail);
    ...
    //vishal changs FCR district I FEE
    public function calculate_fees_hp($itemValues = NULL, $returnCalc = 'N') {
        /*
          1) for Property SD (article=>(Agreemnet,Convenence etc.)) (call after property Save in Citizen Entry, and respective)
          $sd_values = array(
          'token_no'=>9
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
    public function get_calc_fee_rule_list_hp($frm) {
        try {
            $this->autoRender = FALSE;
            $feeRuleList = array();
            $ruleCondition = array();
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
    public function save_fee_calculation_detail_hp($fees_cal_id, $frm, $feeCalcDetail, $articleItems) {
        try {
            $this->autoRender = FALSE;
            ...
            if ($this->fees_calculation_detail->saveAll($fee_calc_detail_final)) {
                return $this->view_fee_calculation_hp($frm['fee_calc_id']);
            } else {
...
//-------------------------------------------------------------------Check Fee Rule Conditions---------------------------------------------------------------------------------
    public function check_fee_condition_hp($articleItems, $frm, $fsrl) {
        try {
// pr($fsrl);
                    ...
                    if (!empty($regconf)) {
                        if ($regconf[0]['conf_reg_bool_info']['info_value'] == 'Y') {
                            ...
                            if ($vamout > $total_cons_amt) {
                             ...
                             if($frm['fee_rule_id']==199||$frm['fee_rule_id']==200||$frm['fee_rule_id']==201){
                            //if ($frm['article_id'] == 89) {
                                if ($frm['FHJ'] == 2) {
                                    if (isset($frm['FBD'])) {
                                        if ($frm['FBD'] > $vamout) {
                                            $frm['FAA'] = $frm['FBD'];
...
//                pr($fsrl);
                if ($fsrl['fee_rule_cond1']) {
                    if (eval("return(" . $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_cond1']) . ");")) {
                        $calculations = $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula1']);
                    } else if ($fsrl['fee_rule_cond2']) {
                        if (eval("return(" . $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_cond2']) . ");")) {
                            $calculations = $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula2']);
                        } else if ($fsrl['fee_rule_cond3']) {
                            if (eval("return(" . $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_cond3']) . ");")) {
                                $calculations = $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula3']);
                            } else if ($fsrl['fee_rule_cond4']) {
                                if (eval("return(" . $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_cond4']) . ");")) {
                                    $calculations = $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula4']);
                                } else if ($fsrl['fee_rule_cond5']) {
                                    $calculations = (eval("return(" . $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_cond5']) . ");")) ? $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula5']) : -1;
                                } else {//if condition 5 not available
                                    $calculations = ($fsrl['fee_rule_formula5']) ? $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula5']) : -1;
                                }
                            } else {//if condition 4  not available
                                $calculations = ($fsrl['fee_rule_formula4']) ? $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula4']) : -1;
                            }
                        } else {// if condition 3  not available
                            $calculations = ($fsrl['fee_rule_formula3']) ? $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula3']) : -1;
                        }
                    } else {//if condition 2 not available
                        $calculations = ($fsrl['fee_rule_formula2']) ? $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula2']) : -1;
                    }
                } else {//condition 1  not Available              
                    $calculations = ($fsrl['fee_rule_formula1']) ? $this->add_value_to_formula_hp($articleItems, $frm, $fsrl['fee_rule_formula1']) : -1;
                }
//                   pr($calculations);
...
//                        $result = ($output_item['article_fee_items']['fee_rounding_flag'] == 'Y') ? round($tmp_result, $tmp_factor) : round($result);
                        ...
                        $rounding_flag_check=$this->article_fee_items->query("select a.rounding_id, b.next_rounding_value from ngdrstab_mst_article_fee_subrule a join ngdrstab_mst_rounding_value b ON a.rounding_id=b.rounding_id  where fee_rule_id=$fee_rule_id and fee_output_item_id=2");
//                        pr($rounding_flag_check);
                        if(!empty($rounding_flag_check))
                        {
                            $value=$rounding_flag_check[0][0]['next_rounding_value'];
                            $result = $this->round_tonext($result, $value);
                        } else {
                            $value = $output_item['rounding']['next_rounding_value'];
                            $result = $this->round_tonext($result, $value);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
    public function add_value_to_formula_hp($itemlist, $value, $formula) {// for replacing item code with value in formula
        try {
            $this->autoRender = FALSE;
//-----------------------for calculated Market Value for multiple Ietm-------------------
            ...
            if ($mv_item_flag == 'N' && $itemlist) {
                ...
                $value['OMV'] = $this->calculate_mv_hp($value);
                if (isset($value['cons_amt'])) {
...
//----------------------------------------------------------------------------------------
    ...
    public function get_certificate_fees_hp() {
        try {
            $this->autoRender = FALSE;
            ...
            if (isset($this->request->data['ctype']) and ( $this->request->data['ctype'] == 'E' or $this->request->data['ctype'] == 'C')) {
                ...
                return $this->calculate_fees_hp($frm, 'Y');
            } else {
                return NULL;
            }
        } catch (Exception $ex) {
...
//------------------***--------------------------------view Calculated Report-----------------------------------------------------------------------------------------
    public function view_fee_calculation_hp($fee_calc_id = NULL, $view_flag = NULL) {
        try {
            $this->autoRender = FALSE;
                            ...
                            foreach ($calcDetailData as $calcDetail) {
                                ...
                                if ($itemtype == 2 or $itemtype == 6) {//for output Items
                                    ...
                                    if ($calcDetail['items']['fee_param_type_id'] == 6) { // for Exemption  Output
                                        ...
                                        if ($calcDetail['fees_calculation_detail']['final_value'] != 0) {
                                            $total += $calcDetail['fees_calculation_detail']['final_value'];
...
//                    $this->set(compact('calc_Data', 'calc_detail'));
    ...
    public function view_sd_calc_hp($doc_token_no = NULL, $fee_type_id = 2, $sess_lang = 'en') {
        try {
            /*
             * Pass $fee_type_id=1 if u want to display only Online Fees;  
             * pass $fee_type_id=2 if u want both Counter and Online Fees;
...
//-------------------------------------------------------------------Update Fee Exemption --------------------------------------------------------------------------------
    public function update_fee_exemption_hp($doc_token_no) {
        try {
...
//------------------***-------------view Exemption--------------------------------------------------------------
    ...
    public function delete_fee_exemption_hp() {
        try {
    ...
    public function view_exemption_hp($doc_token_no = NULL, $sess_lang = NULL) {
        try {
            $this->autoRender = FALSE;
